...
import { BookOpen, Bot, CalendarDays, Check, CheckCircle2, FileText, FolderOpen, GraduationCap, LogOut, Menu, MessageSquare, Mic2, Moon, Play, Search, Sparkles, Trophy, Users, Video, X } from "lucide-react";
  ...
  return (
    <div className="delegate-dashboard min-h-screen bg-[#0f131c] text-[#dfe2ef] selection:bg-cyan-300 selection:text-slate-950">
      ...
      <aside className={`${menuOpen ? "flex" : "hidden"} fixed inset-0 z-40 w-64 flex-col border-r border-white/10 bg-[#181b25] p-4 pt-24 md:flex`}>
        ...
        <nav className="mt-2 space-y-1"><Link href="/committee" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 rounded-xl border border-cyan-300/20 bg-cyan-300/10 px-4 py-3 text-sm font-semibold text-cyan-300 transition hover:border-cyan-300/40 hover:bg-cyan-300/15"><Users className="h-4 w-4" />MUNs</Link><Link href="/ai-doubt-clarifier" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 rounded-xl border border-white/10 px-4 py-3 text-sm font-semibold text-slate-400 transition hover:border-cyan-300/30 hover:bg-white/5 hover:text-cyan-300"><Bot className="h-4 w-4" />AI Doubt Clarifier</Link><Link href="/dashboard/messages" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 rounded-xl border border-white/10 px-4 py-3 text-sm font-semibold text-slate-400 transition hover:border-cyan-300/30 hover:bg-white/5 hover:text-cyan-300"><MessageSquare className="h-4 w-4" />Doubt Clarifier</Link></nav>
